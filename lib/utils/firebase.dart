...
Future<void> initializeFirebase() async {
  ...
  if (!FlavorConfig.isFirebaseEnabled) {
    ...
    log.d(
      '[Firebase] App will run without Firebase features (Crashlytics, etc.)',
    );
    return;
  }
  ...
  if (!kIsWeb && Platform.isLinux) {
    log.d(
      '[Firebase] Linux platform detected - Firebase is not supported natively on Linux',
    );
    log.d('[Firebase] Skipping Firebase initialization');
    log.d(
      '[Firebase] App will run without Firebase features (Crashlytics, etc.)',
  ...
  log.d(
    '[Firebase] Crashlytics status check = $isCrashlyticsCollectionEnabled',
  );
